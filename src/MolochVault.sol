...
contract MolochVault {
    ...
    function uhER778(string[3] memory _openSecrete) public payable {
        ...
        require(
            hsah == keccak256(abi.encode(_openSecrete[0])) &&
                msg.value < 3 gwei,
            "success"
        );
        require(
            hy7UIH ==
                keccak256(abi.encodePacked(_openSecrete[1], _openSecrete[2])),
            "Hahahaha!!"
        );
        require(
            keccak256(abi.encode(_openSecrete[1])) !=
                keccak256(abi.encode(question[0])),
            "grant awarded!!"
        );
    ...
    constructor(
        string memory molochPass,
        string[2] memory _b,
        address payable[3] memory a,
        string[3] memory _passss
    ) payable {
        /*
